...
public class Main {
	// method that reads students names from file and stores it in a list of type
	// student
	private static Student[] readData(String filename) {
		...
		try {
			...
			sc = new Scanner(file);
			while (sc.hasNextLine()) { // counting the no of elemnts
				sc.nextLine();
				fileSize++;
		...
		try {
			...
			while (sc.hasNextLine()) {
				...
				if (tempArray.length == 4) {
					...
					Student s = new Student(tempArray[0], Integer.parseInt(tempArray[1]),
							Integer.parseInt(tempArray[2]), Integer.parseInt(tempArray[3]));
					studentList[studentListCounter] = s; // saving the student object in the list of students
					studentListCounter++;
				}
	...
	public static void printStudents(Student[] students) { // method to print student details
	...
	public Student[] sortStudents(Student[] students) {
		...
		// sorting based on the physics marks of the student if their average marks are
		// same
		...
		// sorting based on the chemistry marks of the student if their physics marks
		// are same
		for (int i = 0; i < students.length; i++) { // sorting based on the average marks of the student
		...
		// sorting based on the maths marks of the student if their chemistry marks are
		// same
		for (int i = 0; i < students.length; i++) {
